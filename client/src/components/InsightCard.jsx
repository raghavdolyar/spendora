...
const typeStyles = {
    monthly_summary: { gradient: 'from-teal-400 to-teal-600', Icon: TrendingUp },
...
const MonthlySummaryView = ({ c }) => (
    <div className="space-y-6">
        <div className="flex flex-col md:flex-row gap-6 items-center bg-linear-to-br from-teal-50 via-white to-blue-50 rounded-2xl p-6 border border-slate-100">
        ...
        {typeof c.estimatedMonthlySavings === 'number' && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <Stat label="Health Score" value={`${c.healthScore ?? 0}/100`} accent="teal" />
                <Stat
            ...
            <div>
                ...
                <div className="space-y-2">
                    {c.recommendations.map((r, i) => (
                        <div
                            key={i}
                            className="p-4 bg-white border border-slate-200 hover:border-teal-200 rounded-2xl transition flex items-start gap-3"
                        >
        ...
        <div className="space-y-5">
            ...
            {totalSavings > 0 && (
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <Stat label="Tips" value={c.tips?.length || 0} accent="teal" />
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {c.tips?.map((t, i) => {
                    ...
                    return (
                        <div
                            key={i}
                            className="group relative p-5 rounded-2xl bg-white border border-slate-100 hover:border-teal-200 hover:shadow-sm transition"
                        >
                ...
                <div>
                    ...
                    <div className="space-y-2">
                        {c.suggestions.map((sug, i) => (
                            <div
                                key={i}
                                className="p-4 bg-white border border-slate-200 hover:border-teal-200 rounded-2xl flex items-start gap-3 transition"
                            >
                                <div className="h-7 w-7 rounded-full bg-teal-50 flex items-center justify-center shrink-0 text-xs font-bold text-teal-700">
                                    {i + 1}
                                </div>
