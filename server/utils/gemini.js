...
export const analyzeTransactionList = async ({
  ...
  const prompt = `Analyze these ${transactions.length} transactions and provide a concise, helpful spending summary.
...
Return ONLY valid JSON (no markdown):
{
  ...
  "highlight": "Single short phrase capturing the key takeaway (e.g., 'Heavy on dining', 'Stable income', 'Dining surge')"
}`;
...
- status: 'good' (well-paced, under target), 'caution' (approaching limit or above 70%), or 'concerning' (over target)
